...
**Nota de negocio — modelo de precio distinto a Canva/Notion:** el plan Team de Sentry **no es por asiento**, es una **tarifa plana con usuarios ilimitados** (confirmado en sentry.io/pricing — tabla de features: "Users: 1 / Unlimited / Unlimited" para Developer/Team/Business — y verificado contra la API de billing real de nuestra propia cuenta: no hay ningún campo de precio por miembro, solo un tope `maxMembers`). Por eso el ahorro de CoStack acá no viene del arbitraje anual/mensual (como en Canva/Notion) sino de **dividir un costo fijo entre los miembros del lobby**: el plan cuesta USD 26/mes (anual) para todo el equipo, y `pricePerMonth` en el catálogo es ese costo total dividido por `availableSeats`. Cuantos más miembros se sumen al lobby, más barato le sale a cada uno — es el ejemplo del catálogo que más se beneficia del Automatch.

La demo corre sobre el **trial gratuito de 14 días** de Sentry (sin tarjeta, features de plan Business durante el trial). La compra real del plan anual queda para cuando haya usuarios pagando de verdad — no se hizo para el MVP.
